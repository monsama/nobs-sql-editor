...
(async () => {
  ...
  try {
    ...
    G.check('Create user makes it with its limits and expiry', /MAX_QUERIES_PER_HOUR 100/.test(made) && /PASSWORD EXPIRE NEVER/i.test(made), made);
    G.check('and its details say so', /at most 100 queries an hour/.test($('userInfo').textContent) && /never expires/.test($('userInfo').textContent), $('userInfo').textContent);
    ...
    answers.push(o => { const r = {}; o.fields.forEach(f => { r[f.key] = f.value; }); r.mq = '0'; r.ssl = 'ANY'; r.locked = true; return r; });
    await acctEdit();
    const edited = await cu("'nobs_ue_a'@'%'");
    G.check('Account settings: SSL required, limit gone, locked', /REQUIRE SSL/i.test(edited) && !/MAX_QUERIES_PER_HOUR 100/.test(edited) && /ACCOUNT LOCK/.test(edited), edited);
